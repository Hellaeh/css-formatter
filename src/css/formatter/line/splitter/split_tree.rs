use std::{
	assert_matches::debug_assert_matches, cell::OnceCell, fmt::Write, hint::unreachable_unchecked,
};
...
/// TLS to reduce allocs
#[thread_local]
static mut ARENA: OnceCell<Vec<SplitNode>> = OnceCell::new();

#[thread_local]
static mut GET_OR_INIT: fn() -> &'static mut Vec<SplitNode> = || {
	unsafe { GET_OR_INIT = unsafe_get_arena };
	unsafe { ARENA.get_mut_or_init(Vec::new) }
};

#[inline]
fn unsafe_get_arena() -> &'static mut Vec<SplitNode> {
	unsafe { ARENA.get_mut().unwrap_unchecked() }
}

impl SplitTree {
	#[inline]
	pub fn from_byte_array(buf: &[u8]) -> Self {
		let splits = unsafe { GET_OR_INIT() };
		let mut split;
		let mut window;
		let mut offset = 0;
		let mut at = 0;
